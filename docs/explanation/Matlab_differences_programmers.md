...
## Hamiltonians and chemical shifts
Pulse sequences allow you to set two distinct "center frequencies". The main reason for this is when the chemical shifts need to be adjusted in the Hamiltonian for some spin systems. This is generally not an issue for proton MRI because 0 ppm is typically defined as the TMS peak and water is at 4.65 ppm and most spectra put water at the center of the spectrum. However, there are other conventions and chemical shifts for other nuclei or other conventions may define frequencies relative to a different point. In these cases, it is still possible to use the spin systems that were defined with 0 ppm as the TMS frequency, but either the shifts themselves should be adjusted in the spinSys dict or the center frequency should be redefined to represent the number on the previous scale. This isn't entirely intuitive (one might want to define the new 0 ppm relative to TMS, but this would only be a suitable definition in the proton case) but it means that the same adjustment can cover this and also cover the case where a spectrum is simulated without water at the center.

(In sim_megapress.m, you see that the shifts for the Hamiltonian are adjusted within the pulse sequence itself, "knowing" that the center frequency for the basis set is 4.65 ppm and using the using this and a second assumption about the spectral center frequency to shift the ppm values after simulation. This is a slightly different way of doing it that accomplishes the same thing. However, in Matlab fid-A, these assumptions about center frequencies are hard-coded rather than function inputs with defaults).
