...
* If you need to add on functionality for dimensions that have not yet been considered or explicitly programmed into pyFidA (eg. indirect dimensions or MRSI), you can just decide on a name for these dimensions and add them to the list in cases where that data is present. While you will need to reference these new dimensions in functions that are affected by them, but there is no need to add them to the dims dict and assign them a value of -1 in every existing function that re-creates the dims dict.
...
* An additional feature is that a list makes it a simple matter to construct a property for the \__contains__ method (these double-underscore or dunder methods are explained more below) that uses the "in" keyword to check whether a dimension is present in the data:
```python
...
Depending on your familiarity with Python, it may not be immediately obvious why an entire class is defined in order to manipulate how arguments are returned. Why not simply define a boolean variable at the top of the pyFidA module and assign it a value? Then let users can assign this variable True or False in order to change the behaviour. This has to do with how/when Python evaluates different pieces of code. When a package is imported, top-level code is executed, while code inside of functions and classes is not. This means that if \_use_default were a boolean value that were assigned True or False directly, then every function where the alter_return_args decorator is applied will check the value of \_use_default when the module loads and that function will then use the value that \_use_default had when the module was imported, regardless of how the user changes it afterward. Put another way, if \use_default=True at the top of the module, then the line "return_flag=_use_default" in the alter_return_args decorator would be replaced with "return_flag=True" when decoration of the function is done on import. Changes to the value of "\_use_default" after that are irrelevant because the True value was already assigned. There are various approaches to dealing with this but making \_use_default an instance of the ReturnBehaviour class and then having the line in alter_return_args be "return_flag=_use_default()" means that the \_use_default() call will happen when the wrapper function (which has the same name as the original function funcnm once the decorator is run) is run and thus the value that is stored in the class is checked at that point rather than using the value from when the module is loaded. This is what allows the user to switch between the two return behaviours. Hence, I have called the class that holds the info ReturnBehaviour. It is probably possible to get this behaviour with a function or other methods that move the evaluation away from the "top level" of the code that is evaluated on import, but creating a class seemed like a relatively clear and Pythonic way of doing it. It also seemed like it would be possible to implement different return behaviours in the future (by adding onto the code in the class) or with different defaults for different modules (by assigning different instances of the class to different variables), but there was no driving reason to implement this in this version of the code.
