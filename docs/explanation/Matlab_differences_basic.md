...
It is also possible to construct your own lineshape functions for use with op_peakFit, as long as they have two input arguments in the form [pars,xvals] but these will not generate a "smart" initial parsGuess. pyFidA will assign every parameter an initial guess of 1 if none is entered as an input argument. In addition the default parameter bounds from parameter_bounds=True cannot be generated since nothing is known about the function's expected parameters. These defaults can be added to op_peakFit, see [the lineshape section in Matlab Differences for Programmers](./Matlab_differences_programmers.md#lineshape).

Finally, op_peakFit will attempt to fit the entire complex spectrum. If, for some reason, you only wish to fit the real portion (to more closely mimic Matlab function, for example, or if you have your own lineshape function that only outputs the real part of the spectrum), you can set real_ydata=True in the input arguments. For any lineshape function that outputs complex data (all of the lineshapes available in pyFidA), there will be a warning that only the real part of the output is being used.
...
### Spin system format ###
If you use the simulation toolbox by loading in .mat files for spin systems (either those from fid-A, contained in the assets folder, or ones that you created yourself for use in Matlab fid-A), most use of simulation tools in pyFidA will be unaffected. You can load these .mat files using pyFidA.read_spinsys_matlab (the metabolites in the assets folder can be accessed simply by their metabolite name - Ala, GPC, etc. - without entering the full filename). Spin systems loaded in this way can then be used as an input to sim_Hamiltonian, which will generate the density matrix and Hamiltonian needed for other operators and pulse sequence simulations in pyFidA. Those function calls to simulate pulse sequences work similarly to those in Matlab, from the user perspective.

However, if you are looking to generate your own spin systems in pyFidA, be aware that a basic spin system is a dict rather than Matlab's struct. The keys correspond to the Matlab struct attributes (name, shifts, J, scaleFactor). For spin systems that have non-interacting spins (eg. GPC), the spin system is a list of dicts with the format described above. The Hamiltonian and density matrix generated from these systems will be lists of Hamiltonian objects and lists of numpy arrays, respectively (vs in Matlab where the Hamiltonian is a struct with length corresponding othe number of elements in the spin system struct and the density matrix is a cell of the same length, containing arrays).
